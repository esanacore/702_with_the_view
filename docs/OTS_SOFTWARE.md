...
| OTS-110 | Pillow | as installed locally | Python Imaging Library contributors | **Local media tooling only** — `tools/optimize_photos.py` generates the WebP derivatives and compresses photos. Not shipped; the site serves the output files, not the library | Low | Output inspected visually; fallback path tested (I-024) so a missing derivative cannot break the page | Python advisories — last reviewed 2026-08-19 | Installed on demand (`pip install Pillow`) | Active |
| OTS-109 | imageio-ffmpeg (bundled FFmpeg binary) | 7.1 | FFmpeg project / imageio | **Local media tooling only** — one-off re-encode of the timelapse and poster extraction. Not shipped, not required to build or test the site | Low | Output verified frame-by-frame against the source before replacing it | FFmpeg security advisories — last reviewed 2026-08-19 | Installed on demand (`pip install imageio-ffmpeg`); removable with `pip uninstall` | Active |
